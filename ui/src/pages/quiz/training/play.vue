...
<template>
    <v-card class="h-100 w-100 position-relative" flat>
        <v-window v-model="currentIndex">
            <v-window-item v-for="(current, i) in quizItems" :key="i">
                <QuesitoView
                    :width="appStore.width"
                    :height="appStore.height"
                    :is-landscape="appStore.isLandscape"
              :domanda="current.domanda" :answer="current.answer" @answer="giveAnsware" @next="next" @pause="onPause"></QuesitoView>
            </v-window-item>
        </v-window>
    </v-card>
</template>
...
import { type Esame, getEsameQuesiti, type PausaEvent, Choice } from '@/services/hankinson';
